    ...
    public static void RenderItemOverlay(net.minecraft.client.gui.DrawContext drawContext, ItemStack stack, GeneratedCardItem gci, int x, int y) {
        if(stack.hasNbt()) {
            ...
            if(card.getBackground() != null) {
                drawContext.drawTexture(card.getBackground(), x, y, 0, 0, 16, 16);
            }
            if(card.getArt() != null) {
                drawContext.drawTexture(card.getArt(), x, y, 0, 0, 16, 16);
            }
            if(card.getIcon() != null) {
                drawContext.drawTexture(card.getIcon(), x, y, 0, 0, 16, 16);
            }
        }
    }
}
